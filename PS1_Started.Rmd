...
On GitHub, generate a project repository for your group for this assignment by cloning the repository from my GitHub account and naming it `name1_name2_name3` where the usernames are the first names of your groupmembers. This repository includes the R-Markdown file you will use to complete the assignment. 
...
and you should secde the directory path to the folder you want to be using.

5) go to github.com/name1_name2_name3/your_group_project and click on the "Clone or Download" button. In the terminal type `git clone` and copy the url it gives you. Paste it as follows:

`$ git clone github.com/name1_name2_name3/your_group_project.git `
...
Open the file in RStudio and click on the `Knit` (to html) button. You should see the incomplete assignment displayed in an html window. You can now make edits to this document. 
...
`$ cd /c/Users/ *******/Dropbox/PS1_GS/your_group_project`
...
4) Now that GitHub knows who to attribute edits to, you can commit the modified files. Type the following:
...
6) You can now look at your repository and check that the edits you made appear in .Rmd file and the .html file. Because others in your group may have made changes as well, GitHub may prompt you to merge changes and fix any conflicting lines.
...
**Now you are set up to work cooperatively on an assignment using GitHub. The remainder of this assignment consists of:**

- Using Github to suggest an edit to my lecture note, slides, or problem set. 

- a simple coding and plotting exercise to practice collaborating with Github.

# Suggest an edit (Each person should submit one of these)

1) Find a typo, edit, or improvement in the lecture notes, slides or problem set that you would like to correct (this could be correcting a spelling mistake, adding titles or labels to a graph or table, or clarifying an explanation).

2) Clone the course Github repository into a folder on your computer. 

3) Because some of the data files are large, some data files are zipped on the github page. Unzip these and place them in the cloned folder (you will need to do this if you wish to compile any of the files that call upon these datasets). Do not remove the zipped file.

4) Make the edits and push your edits to the course repository.

5) I will review your edits and integrate them into my materials. 
